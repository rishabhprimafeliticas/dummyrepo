...
//       // Define processes dynamically
//       const processes = [];
//       if (req.url === "/v2/transfer-token") {
//         processes.push(updateTokenStatusAndTransferAwareTokenBackground(req));
//       } else if (req.url === "/v2/update-transfer-token") {
//         processes.push(updateTokenStatusAndTransferUpdateAwareTokenBackground(req));
//       } else {
//         console.error(`Invalid URL in request for key ${selectedKey}: ${req.url}`);
//         await redisClient.lPop(selectedKey);
//         continue;
//       }

//       try {
//         await redisClient.lPop(selectedKey); // Remove the processed item from Redis

//         // Execute processes sequentially
//         for (const process of processes) {
//           try {
//             await process; // Ensures sequential execution
//           } catch (error) {
//             console.error(`Error while processing key ${selectedKey}:`, error);
//           }
//         }
//       } catch (error) {
//         console.error(`Error while processing key ${selectedKey}:`, error);
...
//   } finally {
//     schedulerRunning = false;
//     await redisClient.quit(); // Close the Redis client properly
//   }
// }
...
//     if (token_status == "approved") {
//       // Handling the approval scenario
//       // var selected_update_aware_token_found = await selected_update_aware_token.findOne({ update_aware_token_id: req.body._id }).catch((ex) => {
//       //   loggerhandler.logger.error(`${ex} ,email:${req.headers.email}`);
//       //   return;
//       // });

//       // var update_physical_assets_found = await update_physical_asset.findOne({ update_aware_token_id: req.body._id }).catch((ex) => {
//       //   loggerhandler.logger.error(`${ex} ,email:${req.headers.email}`);
//       //   return;
//       // });
...
//       };
//     } else {
//       // Handling the rejection scenario
...
//       // Handle unlocking tokens and adding balance
//       await Promise.allSettled(
//         update_physical_assets_found.assetdataArrayMain.map(async (dataset) => {
//           await callstack.unlockedWTheLockedTokensAndAddingBalance(
//             dataset,
//             req
//           );
//         })
...
//           },
//           { status: "Rejected", message: req.body.message },
//           { new: true }
//         )
//         .catch((ex) => {
//           loggerhandler.logger.error(`${ex} ,email:${req.headers.email}`);
...
//       // Commenting out web3_handler.updateAwareTokenAsync() for testing
//       /*
//       const updateTokenResult = await new Promise((resolve, reject) => {
//         web3_handler.updateAwareTokenAsync(_id, async function (response) {
//           if (response == false) {
...
//             const notificationcerated = await notifications.create({
//               notification_sent_to: aware_id,
//               message: `Your ${assets_avaliable.weight} tokens are approved by your manager. You can send them now.`,
//             });
//             console.log("Notification created", notificationcerated);
...
//       // For testing, we set the update response manually
//       const updateTokenResult = {
//         success: true,
//         message: "Token update successful.",
//       };

//       // Execute the code that would have been executed in the callback on a successful response
//       let product_line = temp_product_lines_avaliable?.product_line;
//       if (temp_product_lines_avaliable) {
//         let elementIndex = product_line.findIndex(
//           (obj) => obj.update_aware_token_id == req.body._id
...
//           .findOneAndUpdate(
//             {
//               _id: mongoose.Types.ObjectId(temp_product_lines_avaliable?.po_id),
//               deleted: false,
//             },
//             { locked: false }
//           )
