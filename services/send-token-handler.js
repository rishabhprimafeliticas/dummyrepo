...
exports.handlers = {
              ...
              for (var i = 0; i < send_aw_tokens_avaliable.length; i++) {
                ...
                temp_selected_aware_token_avaliable?.selected_tokens.forEach(
                  (ele) => {
                    ...
                    // return ({ ...ele.toObject(), color: color })
                  }
                );
                // if (token_avaliable?.length > 0) {
                // console.log({token_avaliable})

                //   temp_selected_aware_token_avaliable.selected_tokens =[]
                //   temp_selected_aware_token_avaliable.selected_tokens = token_avaliable
  ...
  //     return res.status(422).jsonp(errors.array());
  //   } else {
  //     // if (!req.headers.userid || !req.headers.username || !req.headers.authorization || !req.headers._awareid) {
  ...
  //       if (resp.status == true) {
  //         try {
  //           var sendAwTokensAvailable = await send_aw_tokens.find({ _awareid: req.headers.awareid, status: { $ne: "Approved" } });
  //           if (sendAwTokensAvailable.length <= 0) {
  //             return res.status(200).jsonp({ status: true, data: null, authorization: resp.token });
  //           } else {
  //             var [
  //               selectedReceiverAvailable,
  //               selectedAwareTokenAvailable,
  //               selectedTransactionCertificatesAvailable,
  //               selectedProofOfDeliveryAvailable,
  //               physicalAssetAvailable,
  //               updatePhysicalAssetAvailable
  //             ] = await Promise.all([
  //               select_receiver.find({}),
  //               selected_aware_token.find({}),
  //               selected_transaction_certificates.find({}),
  //               selected_proof_of_delivery.find({}),
  //               physical_assets.find({}),
  //               update_physical_asset.find({})
  //             ]);
  //             console.log({
  //               selectedReceiverAvailable,
  //               selectedAwareTokenAvailable,
  ...
  //             })
  //             // var jsonData = [];
  //             // for (var i = 0; i < sendAwTokensAvailable.length; i++) {
  //             //     var tempAwToken = sendAwTokensAvailable[i];
  //             //     var tempSelectedReceiver = selectedReceiverAvailable.find(x => x._awareid == tempAwToken._awareid && x.send_aware_token_id == tempAwToken._id);
  //             //     var tempSelectedAwareToken = selectedAwareTokenAvailable.find(x => x._awareid == tempAwToken._awareid && x.send_aware_token_id == tempAwToken._id);
  //             //     var tempSelectedTransactionCertificates = selectedTransactionCertificatesAvailable.find(x => x._awareid == tempAwToken._awareid && x.send_aware_token_id == tempAwToken._id);
  //             //     var tempSelectedProofOfDelivery = selectedProofOfDeliveryAvailable.find(x => x._awareid == tempAwToken._awareid && x.send_aware_token_id == tempAwToken._id);

  //             //     let tokenAvailable = tempSelectedAwareToken?.selected_tokens.map((ele) => {
  //             //         let color;
  //             //         if (ele.aware_token_id) {
  //             //             color = (physicalAssetAvailable.find(x => x.aware_token_id == ele.aware_token_id))?.main_color || null;
  //             //         } else if (ele.update_aware_token_id) {
  //             //             color = (updatePhysicalAssetAvailable.find(x => x.update_aware_token_id == ele.update_aware_token_id))?.main_color || null;
  //             //         }
  //             //         return ({ ...ele, color: color });
  //             //     });
  //             //     if (tempSelectedAwareToken) {
  //             //       tempSelectedAwareToken.selected_tokens = token_avaliable.length > 0 ? token_avaliable : [];
  //             //   }
  //             //     var jsonObject = {
  //             //         "send_aw_tokens": tempAwToken,
  //             //         "selected_receiver_available": tempSelectedReceiver ? tempSelectedReceiver : null,
  //             //         "selected_aware_token_available": tempSelectedAwareToken ? tempSelectedAwareToken : null,
  //             //         "selected_transaction_certificates_available": tempSelectedTransactionCertificates ? tempSelectedTransactionCertificates : null,
  //             //         "selected_proof_of_delivery_available": tempSelectedProofOfDelivery ? tempSelectedProofOfDelivery : null
  //             //     };
        ...
        function (resp) {
          if (resp.status == true) {
            if (selected_transaction_certificates_exist) {
              ...
              //   "tc_date": req.body.tc_date,
              //   "tc_number": req.body.tc_number,
              //   "certificate_doc": req.file != undefined ? req.file.filename.replace(/\s/g, "") : req.body.file_already_exist
              // }
              ...
              selected_proof_of_delivery.findOneAndUpdate(
                ...
                },
                {
                  ...
                  material_certificates: material_certificates
                    ...
                    : null,
                  labtests_pdf: req.files.labtests
                    ? req.files.labtests[0].filename
                    : selected_proof_of_delivery_exist.labtests_pdf,
                  billoflading_pdf: req.files.billoflading
                    ? req.files.billoflading[0].filename
                    : selected_proof_of_delivery_exist.billoflading_pdf,
                  modeoftransport: req.body.modeoftransport
                    ? req.body.modeoftransport
                    : selected_proof_of_delivery_exist.modeoftransport,
                },
                { new: true },
                async function (err, user) {
                  if (err) {
  ...
  getAllSendAwareTokenRequestsAsync: async (req, res) => {
    ...
    // Build filter for send_aw_tokens: always exclude tokens with status "concept" (case-insensitive)
  ...
  //             // Stage 6: Lookup selected_transaction_certificates
  //             {
  //               $lookup: {
  //                 from: "selected_transaction_certificates",
  //                 let: { awareid: "$_awareid", tokenid: "$_id" },
  //                 pipeline: [
  //                   {
  //                     $match: {
  ...
  //                   },
  //                 ],
  //                 as: "selected_transaction_certificates",
  //               },
  //             },

  //             // Stage 7: Lookup selected_proof_of_delivery
  //             {
  //               $lookup: {
  //                 from: "selected_proof_of_deliveries",
  //                 let: { awareid: "$_awareid", tokenid: "$_id" },
  //                 pipeline: [
  //                   {
  //                     $match: {
  ...
  //             // Stage 9: Format the output - convert arrays to single objects or null
  //             {
  //               $addFields: {
  //                 selected_aware_token_avaliable: {
  //                   $cond: {
  ...
  //                     if: {
  //                       $gt: [
  //                         { $size: "$selected_transaction_certificates" },
  //                         0,
  //                       ],
  //                     },
  //                     then: {
  //                       $arrayElemAt: ["$selected_transaction_certificates", 0],
  //                     },
  //                     else: null,
  //                   },
  //                 },
  ...
  //                   $cond: {
  //                     if: {
  //                       $gt: [{ $size: "$selected_proof_of_delivery" }, 0],
  //                     },
  //                     then: {
  //                       $arrayElemAt: ["$selected_proof_of_delivery", 0],
  //                     },
  //                     else: null,
  //                   },
  //                 },
  ...
  //             // Stage 10: Create final structure
  //             {
  //               $project: {
  //                 send_aw_tokens: "$$ROOT",
  //                 selected_aware_token_avaliable: 1,
  //                 selected_receiver_avaliable: 1,
  //                 selected_transaction_certificates_avaliable: 1,
  //                 selected_proof_of_delivery_avaliable: 1,
  //                 selected_sender_company_name: 1,
  //                 selected_receiver_company_name: 1,
  //               },
  //             },

  //             // Stage 11: Remove unwanted nested fields from send_aw_tokens
  //             {
  //               $unset: [
  //                 "send_aw_tokens.selected_aware_token",
  //                 "send_aw_tokens.kyc_detail",
  //                 "send_aw_tokens.selected_receiver",
  //                 "send_aw_tokens.selected_transaction_certificates",
  //                 "send_aw_tokens.selected_proof_of_delivery",
  //                 "send_aw_tokens.selected_aware_token_avaliable",
  //                 "send_aw_tokens.selected_receiver_avaliable",
  //                 "send_aw_tokens.selected_transaction_certificates_avaliable",
  //                 "send_aw_tokens.selected_proof_of_delivery_avaliable",
  //                 "send_aw_tokens.selected_sender_company_name",
  //                 "send_aw_tokens.selected_receiver_company_name",
  //               ],
  //             },

  //             // Stage 12: Sort by created_date descending (newest first)
  //             {
  //               $sort: {
  //                 "send_aw_tokens.created_date": -1,
  //               },
  ...
  //             // Stage 13: Get total count for pagination
  //             {
  //               $facet: {
  //                 metadata: [{ $count: "total" }],
  //                 data: [{ $skip: skip }, { $limit: limit }],
  ...
  //   // Build filter for send_aw_tokens: always exclude tokens with status "concept" (case-insensitive)
  //   const conceptRegex = new RegExp("^concept$", "i");
  //   let sendAwTokensFilter = { status: { $not: conceptRegex } };

  //   // Build filter for selected_aware_token: search on aware_token_type and selected tokens fields
  //   let selectedAwareTokenFilter = {};
  //   if (searchTerm.length > 0) {
  //     selectedAwareTokenFilter.$or = [
  //       { aware_token_type: new RegExp(escapedSearchTerm, "i") },
  //       { asset_id: new RegExp(escapedSearchTerm, "i") },
  //       { update_asset_id: new RegExp(escapedSearchTerm, "i") },
  //       { to_be_send: new RegExp(escapedSearchTerm, "i") },
  //     ];
  //   }

  //   // Build filter for KYCs: search by company_name
  //   let kycsFilter = {};
  //   if (searchTerm.length > 0) {
  //     kycsFilter.company_name = new RegExp(escapedSearchTerm, "i");
  //   }
  ...
  //     async function (resp) {
  //       if (resp.status === true) {
  //         // Get total count for pagination on send_aw_tokens
  //         let totalDocuments = await send_aw_tokens
  //           .countDocuments(sendAwTokensFilter)
  //           .catch((ex) => {
  //             loggerhandler.logger.error(`${ex} ,email:${req.headers.email}`);
  //             return res
  //               .status(400)
  ...
  //         // Retrieve send_aw_tokens with pagination and filter applied
  //         var send_aw_tokens_avaliable = await send_aw_tokens
  //           .find(sendAwTokensFilter)
  //           .skip(skip)
  //           .limit(limit)
  //           .catch((ex) => {
  //             loggerhandler.logger.error(`${ex} ,email:${req.headers.email}`);
  ...
  //         if (
  //           !send_aw_tokens_avaliable ||
  //           send_aw_tokens_avaliable.length <= 0
  //         ) {
  //           return res.status(200).jsonp({
  //             status: true,
  //             data: null,
  ...
  //           var kycs_avaliable = await kyc_details
  //             .find(kycsFilter)
  ...
  //                   x.send_aware_token_id == temp_aw_token._id
  //               );
  //             var temp_selected_sender_kyc = null;
  //             var temp_selected_receiver_kyc = null;
  //             if (temp_selected_receiver_avaliable) {
  //               temp_selected_sender_kyc = kycs_avaliable.find(
  //                 (x) => x.aware_id == temp_selected_receiver_avaliable._awareid
  //               );
  //               temp_selected_receiver_kyc = kycs_avaliable.find(
  //                 (x) =>
  //                   x.aware_id ==
  //                   temp_selected_receiver_avaliable._receiver_awareid
  //               );
  //             }
  ...
  //               send_aw_tokens: temp_aw_token,
  //               selected_receiver_avaliable:
  //                 temp_selected_receiver_avaliable || null,
  //               selected_aware_token_avaliable:
  //                 temp_selected_aware_token_avaliable || null,
  //               selected_transaction_certificates_avaliable:
  //                 temp_selected_transaction_certificates_avaliable || null,
  //               selected_proof_of_delivery_avaliable:
  //                 temp_selected_proof_of_delivery_avaliable || null,
  //               selected_sender_company_name: temp_selected_sender_kyc || null,
  //               selected_receiver_company_name:
  //                 temp_selected_receiver_kyc || null,
  //             };
  ...
  //         var send_aw_tokens_avaliable = await send_aw_tokens.find({}).catch((ex) => { loggerhandler.logger.error(`${ex} ,email:${req.headers.email}`); return res.status(400).jsonp({ status: false, message: "Bad request!" }); })
  ...
  //           var kycs_avaliable = await kyc_details.find({}).select(["aware_id", "company_name"]).catch((ex) => { loggerhandler.logger.error(`${ex} ,email:${req.headers.email}`); return res.status(400).jsonp({ status: false, message: "Bad request!" }); })

  //           var selected_receiver_avaliable = await select_receiver.find({}).catch((ex) => { loggerhandler.logger.error(`${ex} ,email:${req.headers.email}`); return res.status(400).jsonp({ status: false, message: "Bad request!" }); })

  //           var selected_aware_token_avaliable = await selected_aware_token.find({}).catch((ex) => { loggerhandler.logger.error(`${ex} ,email:${req.headers.email}`); return res.status(400).jsonp({ status: false, message: "Bad request!" }); })

  //           var selected_transaction_certificates_avaliable = await selected_transaction_certificates.find({}).catch((ex) => { loggerhandler.logger.error(`${ex} ,email:${req.headers.email}`); return res.status(400).jsonp({ status: false, message: "Bad request!" }); })

  //           var selected_proof_of_delivery_avaliable = await selected_proof_of_delivery.find({}).catch((ex) => { loggerhandler.logger.error(`${ex} ,email:${req.headers.email}`); return res.status(400).jsonp({ status: false, message: "Bad request!" }); })
  //           var jsonData = [];
  ...
  //             var temp_selected_receiver_avaliable = selected_receiver_avaliable.find(x => x._awareid == temp_aw_token._awareid && x.send_aware_token_id == temp_aw_token._id);
  //             var temp_selected_aware_token_avaliable = selected_aware_token_avaliable.find(x => x._awareid == temp_aw_token._awareid && x.send_aware_token_id == temp_aw_token._id)
  //             var temp_selected_transaction_certificates_avaliable = selected_transaction_certificates_avaliable.find(x => x._awareid == temp_aw_token._awareid && x.send_aware_token_id == temp_aw_token._id)
  //             var temp_selected_proof_of_delivery_avaliable = selected_proof_of_delivery_avaliable.find(x => x._awareid == temp_aw_token._awareid && x.send_aware_token_id == temp_aw_token._id)
  //             var temp_selected_sender_kyc = null;
  //             var temp_selected_receiver_kyc = null;
  //             if (temp_selected_receiver_avaliable) {
  //               temp_selected_sender_kyc = kycs_avaliable.find(x => x.aware_id == temp_selected_receiver_avaliable._awareid);
  //               temp_selected_receiver_kyc = kycs_avaliable.find(x => x.aware_id == temp_selected_receiver_avaliable._receiver_awareid);
  //             }

  //             var jsonObject = {
  //               "send_aw_tokens": temp_aw_token,
  //               "selected_receiver_avaliable": temp_selected_receiver_avaliable ? temp_selected_receiver_avaliable : null,
  //               "selected_aware_token_avaliable": temp_selected_aware_token_avaliable ? temp_selected_aware_token_avaliable : null,
  //               "selected_transaction_certificates_avaliable": temp_selected_transaction_certificates_avaliable ? temp_selected_transaction_certificates_avaliable : null,
  //               "selected_proof_of_delivery_avaliable": temp_selected_proof_of_delivery_avaliable ? temp_selected_proof_of_delivery_avaliable : null,
  //               "selected_sender_company_name": temp_selected_sender_kyc ? temp_selected_sender_kyc : null,
  //               "selected_receiver_company_name": temp_selected_receiver_kyc ? temp_selected_receiver_kyc : null,
  //             }
  ...
  //   }
  //   else {
  //     var selected_aware_token_avaliable = await selected_aware_token.findOne({ _awareid: req.body._awareid, send_aware_token_id: req.body.send_aware_token_id }).catch((ex) => { loggerhandler.logger.error(`${ex} ,email:${req.headers.email}`); return res.status(400).jsonp({ status: false, message: "Bad request!" }); })

  //     let staticid = ['65b8cbf20656432fac7c60ca', '65b8cbf20656432fac7c61ca', '65b8cbf20656432fac7c62ca', '65b8cbf20656432fac7c63ca', '65b8cbf20656432fac7c64ca', '65b8cbf20656432fac7c65ca', '65b8cbf20656432fac7c66ca', '65b8cbf20656432fac7c67ca', '65b8cbf20656432fac7c68ca', '65b8cbf20656432fac7c69ca']
  //     let type = (staticid.includes(req.body.send_aware_token_id) || false)

  //     var select_receiver_avaliable = await select_receiver.findOne({ _awareid: req.body._awareid, send_aware_token_id: req.body.send_aware_token_id }).catch((ex) => { loggerhandler.logger.error(`${ex} ,email:${req.headers.email}`); return res.status(400).jsonp({ status: false, message: "Bad request!" }); })
  ...
  //     var wallet_of_sender = await wallets.findOne({ _awareid: select_receiver_avaliable._awareid }).select(['wallet_address_0x', 'from', 'to']).catch((ex) => { loggerhandler.logger.error(`${ex} ,email:${req.headers.email}`); return res.status(400).jsonp({ status: false, message: "Bad request!" }); })
  //     var wallet_of_receiver = await wallets.findOne({ _awareid: select_receiver_avaliable._receiver_awareid }).select(['wallet_address_0x', 'from', 'to']).catch((ex) => { loggerhandler.logger.error(`${ex} ,email:${req.headers.email}`); return res.status(400).jsonp({ status: false, message: "Bad request!" }); })
  ...
  //     var count_of_transactions = await connectWeb3.eth.getTransactionCount(from0xaddress);
  ...
  //     if (promise_failed) {
  //       return res.status(400).jsonp({ status: false, message: 'Transaction has been reverted!', authorization: null });
  //     }

  //     //importan code to check ownership and balacne
  //     // var check_balance = await contract.methods.ownerOf(token_ids[0]).call();
  //     // console.log("check_balance", check_balance);
  //     // const balance_admin = await contract.methods.balanceOf(from0xaddress.toLowerCase(), token_ids[0]).call();;
  //     // const balance_user = await contract.methods.balanceOf(useraddress.toLowerCase(), token_ids[0]).call();;

  //     // console.log("from0xaddress.toLowerCase()",from0xaddress.toLowerCase())
  ...
  //     var gasAmount = await contract.methods.safeBatchTransferFrom(from0xaddress.toLowerCase(), useraddress.toLowerCase(), token_ids, array_of_amounts, []).estimateGas({ from: from0xaddress });
  ...
  //         console.log("Balance while send aw update token", iotxBalance.toFixed(18))

  //         const senderKycDetails = await kyc_details.findOne({ aware_id: select_receiver_avaliable._awareid }).catch((ex) => { return res.status(400).jsonp({ status: false, message: "Bad request!" }); })
  //         const receiverKycDetails = await kyc_details.findOne({ aware_id: select_receiver_avaliable._receiver_awareid }).catch((ex) => { return res.status(400).jsonp({ status: false, message: "Bad request!" }); })
  //         const sender_details = await account_details.findOne({ kyc_id: senderKycDetails?._id }).catch((ex) => { return res.status(400).jsonp({ status: false, message: "Bad request!" }); })
  //         const receiver_details = await account_details.findOne({ kyc_id: receiverKycDetails?._id }).catch((ex) => { return res.status(400).jsonp({ status: false, message: "Bad request!" }); })

  //         let object = {
  //           senderKycDetails, receiverKycDetails,
  ...
  //           await transferAsync(from0xaddress, gasAmount,
  ...
  //                   let iotxBalance = Big(balance).div(10 ** 18);
  //                   console.log("Balance after t/f of funds send aw token", iotxBalance.toFixed(18))
  ...
  //                       data: contract.methods.safeBatchTransferFrom(from0xaddress.toLowerCase(), useraddress.toLowerCase(), token_ids, array_of_amounts, []).encodeABI(),
  //                     };
  ...
  //                             selected_aware_token_avaliable.selected_tokens.map(async (dataset) => {
  //                               await callstack.sendTokenAndUpdateBalanceInBatch(dataset, select_receiver_avaliable, selected_aware_token_avaliable, receipt, req);
  //                             })
  //                           );

  //                           var promise_failed = result_promise.find(x => x.status == 'rejected');
  //                           if (promise_failed) {
  //                             return res.status(400).jsonp({ status: false, message: 'Transaction has been reverted!', authorization: null });
  //                           }

  //                           var send_aw_tokens_avaliable = await send_aw_tokens.find({ _awareid: req.body._awareid, status: "CONCEPT", create_token_stepper: 1 }).catch((ex) => { loggerhandler.logger.error(`${ex} ,email:${req.headers.email}`); return res.status(400).jsonp({ status: false, message: "Bad request!" }); })
  ...
  //                           await send_aw_tokens.findOneAndUpdate({ _awareid: req.body._awareid, _id: mongoose.Types.ObjectId(req.body.send_aware_token_id) }, { status: 'SEND' }, { new: true }).catch((ex) => { loggerhandler.logger.error(`${ex} ,email:${req.headers.email}`); return res.status(400).jsonp({ status: false, message: "Bad request!" }); })

  //                           await send_aw_tokens.deleteMany({ _id: { $in: output } }).catch((ex) => { return res.status(400).jsonp({ status: false, message: "Bad request!" }); })
  //                           if (req.body.po_id) {

  //                             if (type) {
  //                               await purchase_orders.findOneAndUpdate({ _id: mongoose.Types.ObjectId(req.body.po_id) , deleted:false}, { locked: false }).catch((ex) => { loggerhandler.logger.error(`${ex} ,email:${req.headers.email}`); return res.status(400).jsonp({ status: false, message: "Bad request!" }); })
  //                               var product_lines_avaliable = await product_lines.findOne({ po_id: req.body.po_id }).catch((ex) => { loggerhandler.logger.error(`${ex} ,email:${req.headers.email}`); return res.status(400).jsonp({ status: false, message: "Bad request!" }); })
  ...
  //                                   let check = selected_aware_token_avaliable.selected_tokens.find(e => e.update_aware_token_id == ele.update_aware_token_id)
  //                                   if (approved_line && check) { ele.update_status = 'FILLED' }
  //                                 })
  //                                 await product_lines.findOneAndUpdate({ po_id: req.body.po_id }, { product_line: product_line }, { new: true }).catch((ex) => { loggerhandler.logger.error(`${ex} ,email:${req.headers.email}`); return res.status(500).jsonp({ status: false, message: ex.toString() }) })
  //                                 let validate = product_line.every((e) => e.update_status == 'FILLED')
  //                                 if (validate) { await purchase_orders.findOneAndUpdate({ _id: mongoose.Types.ObjectId(req.body.po_id), deleted:false }, { status: 'FILLED' }, { new: true }).catch((ex) => { loggerhandler.logger.error(`${ex} ,email:${req.headers.email}`); return res.status(500).jsonp({ status: false, message: ex.toString() }) }) }
  //                               }
  //                             } else {
  //                               await purchase_orders.findOneAndUpdate({ _id: mongoose.Types.ObjectId(req.body.po_id) , deleted:false}, { locked: false }).catch((ex) => { loggerhandler.logger.error(`${ex} ,email:${req.headers.email}`); return res.status(400).jsonp({ status: false, message: "Bad request!" }); })
  //                               var product_lines_avaliable = await product_lines.findOne({ po_id: req.body.po_id }).catch((ex) => { loggerhandler.logger.error(`${ex} ,email:${req.headers.email}`); return res.status(400).jsonp({ status: false, message: "Bad request!" }); })
  ...
  //                                 product_line.forEach((ele) => {
  //                                   let approved_line = ele.update_status == 'APPROVED';
  //                                   if (approved_line) { ele.update_status = 'FILLED' }
  //                                 })
  //                                 await product_lines.findOneAndUpdate({ po_id: req.body.po_id }, { product_line: product_line }, { new: true }).catch((ex) => { loggerhandler.logger.error(`${ex} ,email:${req.headers.email}`); return res.status(500).jsonp({ status: false, message: ex.toString() }) })
  //                                 let validate = product_line.every((e) => e.update_status == 'FILLED')
  //                                 if (validate) { await purchase_orders.findOneAndUpdate({ _id: mongoose.Types.ObjectId(req.body.po_id), deleted:false }, { status: 'FILLED' }, { new: true }).catch((ex) => { loggerhandler.logger.error(`${ex} ,email:${req.headers.email}`); return res.status(500).jsonp({ status: false, message: ex.toString() }) }) }
  ...
  //                           }
  //                           // const senderKycDetails = await kyc_details.findOne({ aware_id: select_receiver_avaliable._receiver_awareid }).catch((ex) => { return res.status(400).jsonp({ status: false, message: "Bad request!" }); })
  //                           // const receiverKycDetails = await kyc_details.findOne({ aware_id: select_receiver_avaliable._awareid }).catch((ex) => { return res.status(400).jsonp({ status: false, message: "Bad request!" }); })
  //                           // const sender_details = await account_details.findOne({ kyc_id: senderKycDetails?._id }).catch((ex) => { return res.status(400).jsonp({ status: false, message: "Bad request!" }); })
  //                           // const receiver_details = await account_details.findOne({ kyc_id: senderKycDetails?._id }).catch((ex) => { return res.status(400).jsonp({ status: false, message: "Bad request!" }); })
  ...
  //                           refresh(req.headers.authorization, req.headers.userid, payload, async function (resp) {
  //                             if (resp.status == true) {
  //                               // SendGrid.sendCryptoTcMail(object, (result) => {
  //                               //   if (result != null) {
  //                               //     return res.status(200).jsonp({ status: true, message: "Tokens have been transferred successfully", authorization: resp.token });
  //                               //   } else {
  //                               //     return res.status(500).jsonp({ status: false, message: null, authorization: null });
  ...
  //                           checkTransactionReceipt(connectWeb3, signedTx.transactionHash, async function (result) {
  ...
  //                                   if (product_lines_avaliable) {
  //                                     var product_line = product_lines_avaliable.product_line;
  //                                     // product_line.forEach((ele) => { let approved_line = ele.update_status == 'APPROVED'; if (approved_line) { ele.update_status = 'FILLED' } })
  //                                     product_line.forEach((ele) => {
  //                                       let approved_line = ele.update_status == 'APPROVED';
  //                                       let check = selected_aware_token_avaliable.selected_tokens.find(e => e.update_aware_token_id == ele.update_aware_token_id)
  //                                       if (approved_line && check) { ele.update_status = 'FILLED' }
  ...
  //     //code without send token
  //     // var result_promise = await Promise.allSettled(
  //     //   selected_aware_token_avaliable.selected_tokens.map(async (dataset) => {
  //     //     await callstack.sendTokenAndUpdateBalanceInBatchTemp(dataset, select_receiver_avaliable, selected_aware_token_avaliable, req);
  //     //   })
  //     // );
  ...
  postRecapAsync: async (req, res) => {
    ...
    if (!errors.isEmpty()) {
      ...
      for (var i = 0; i < temp_array.length; i++) {
        ...
        // const balance_admin = await contract.methods.balanceOf(from0xaddress.toLowerCase(), temp.blockchain_id).call();;
        ...
        // console.log("check_balance", temp.To_be_Send);
                          ...
                          connectWeb3.eth
                            ...
                            .on("receipt", async function (receipt) {
                              ...
                              if (req.body.po_id) {
                                ...
                                // var product_lines_avaliable = await product_lines.findOne({ po_id: req.body.po_id ,deleted:false}).catch((ex) => { loggerhandler.logger.error(`${ex} ,email:${req.headers.email}`); return res.status(400).jsonp({ status: false, message: "Bad request!" }); })
                                  ...
                                  if (result == true) {
                                    ...
                                    refresh(
                                      ...
                                      async function (resp) {
                                        if (resp.status == true) {
                                          // await notifications.create({
                                          //   notification_sent_to: select_receiver_avaliable._receiver_awareid,
                                          //   message: `${senderKycDetails?.company_name} sent tokens to you`
                                          // })
      ...
      refresh(
        ...
        async function (resp) {
          if (resp.status == true) {
            ...
            // console.log("select_receiver_avaliable", select_receiver_avaliable)
            // console.log("selected_aware_token_avaliable", selected_aware_token_avaliable)
            // console.log("selected_transaction_certificates_avaliable", selected_transaction_certificates_avaliable)
            // console.log("selected_proof_of_delivery_avaliable", selected_proof_of_delivery_avaliable)
  ...
  getCryptoTcDetailsRequestAsync: async (req, res) => {
    try {
      ...
      if (cryptoDetail.select_receiver_avaliable) {
        ...
        // object.sender_details = sender_details;
        // object.receiver_details = receiver_details
      ...
      // SendGrid.sendCryptoTcMail(object, (result) => {
      //   if (result != null) {
      //     return res.status(200).jsonp({ status: true, data: { cryptoDetail, DataJson, url, object } });

      //     // return res.status(200).jsonp({ status: true, message: "Tokens have been transferred successfully" });
      //   } else {
  ...
  getMaterialCertificatesAsync: async (req, res) => {
    ...
    if (!errors.isEmpty()) {
      ...
      refresh(
        ...
        async function (resp) {
          if (resp.status == true) {
            // var avaliable_material_certificates = await material_certificates.find({}).catch((ex) => { loggerhandler.logger.error(`${ex} ,email:${req.headers.email}`); return res.status(400).jsonp({ status: false, message: "Bad request!", authorization: resp.token }); })

            // var avaliable_material_certificates = await material_certificates
            //   .find({})
            //   // .populate('type') // Populate the 'type' field with data from 'material_certificate_types'
            //   .catch((ex) => {
            //     console.log("ex",ex)
            //     loggerhandler.logger.error(`${ex} ,email:${req.headers.email}`);
            //     // return res.status(400).jsonp({ status: false, message: "Bad request!", authorization: resp.token });
            //   });

            // console.log("avaliable_material_certificates", avaliable_material_certificates)
            // return res.status(200).jsonp({ status: true, data: avaliable_material_certificates });
...
// async function longTask(token_details, select_receiver_avaliable, selected_aware_token_avaliable) {
//   return new Promise(async (resolve, reject) => {
//     try {

//       console.log(token_details, select_receiver_avaliable, selected_aware_token_avaliable)

//       // var wallet = await wallets.findOne({ user_id: record.user_id }).catch(async (ex) => {

//       //   await health_goals_status_directories.findOneAndUpdate({ _id: mongoose.Types.ObjectId(record._id) }, {
//       //     reward_status: false,
//       //   }).catch((ex) => { reject(); });

//       //   await earn_bonus_status_directories.findOneAndUpdate({ _id: mongoose.Types.ObjectId(record._id) }, {
//       //     reward_status: false,
//       //   }).catch((ex) => { reject(); });
