...
exports.handlers = {
    ...
    if (!errors.isEmpty()) {
      ...
      refresh(
        ...
        async function (resp) {
          if (resp.status == true) {
            try {
              const concept_tokens = await send_aw_tokens
                .find({
              ...
              const send_tokens = await send_aw_tokens
                .find({
                  _awareid: req.headers.awareid,
                  status: "SEND",
                  hide: { $ne: true },
              ...
              const other_tokens = await send_aw_tokens
                .find({
                  _awareid: req.headers.awareid,
                  status: { $nin: ["Approved", "CONCEPT", "SEND"] },
                  hide: { $ne: true },
                })
                .sort({ created_date: -1 })
                .catch((ex) => {
              ...
              const jsonData = [];
              for (let i = 0; i < send_aw_tokens_avaliable.length; i++) {
                const temp_aw_token = send_aw_tokens_avaliable[i];

                const temp_selected_receiver_avaliable =
                  selected_receiver_avaliable.find(
                    (x) =>
                      x._awareid == temp_aw_token._awareid &&
                      x.send_aware_token_id == temp_aw_token._id
                  );
                const temp_selected_aware_token_avaliable =
                  selected_aware_token_avaliable.find(
                    (x) =>
                      x._awareid == temp_aw_token._awareid &&
                      x.send_aware_token_id == temp_aw_token._id
                  );
                const temp_selected_transaction_certificates_avaliable =
                  selected_transaction_certificates_avaliable.find(
                    (x) =>
                      x._awareid == temp_aw_token._awareid &&
                      x.send_aware_token_id == temp_aw_token._id
                  );
                const temp_selected_proof_of_delivery_avaliable =
                ...
                if (temp_selected_aware_token_avaliable?.selected_tokens) {
  ...
  selectAwareTokenAsync: async (req, res) => {
    ...
    if (!errors.isEmpty()) {
      ...
      const seletedTokens = req.body.selected_tokens;
      for (let i = 0; i < seletedTokens.length; i++) {
        const token = seletedTokens[i];
        if (token.aware_token_id) {
          const tokenExist = await aw_tokens
            .findOne({ _id: token.aware_token_id })
            .select("locked");
          if (tokenExist && tokenExist.locked) {
            return res
              .status(401)
              .jsonp({ status: false, message: "Token is already locked!" });
          }
        } else if (token.update_aware_token_id) {
          const tokenExist = await update_aw_tokens
            .findOne({ _id: token.update_aware_token_id })
            .select("locked");
          if (tokenExist && tokenExist.locked) {
            return res
              .status(401)
            ...
            if (selected_aware_token_exist) {
              ...
              for (const item of req.body.selected_tokens) {
                if (!map.has(mongoose.Types.ObjectId(item.aware_token_id))) {
                  map.set(mongoose.Types.ObjectId(item.aware_token_id), true);
                            ...
                            .on("receipt", async function (receipt) {
                              ...
                              if (req.body.po_id) {
                                ...
                                if (product_lines_avaliable) {
                                  ...
                                  let deletedLines = product_line.filter(
                                    (e) => e.deleted
                                  );
                                  let nonDeletedLines = product_line.filter(
                                    (e) => !e.deleted
                                  );

                                  nonDeletedLines.forEach((ele) => {
                                    if (ele.update_status == "APPROVED") {
                                      length += 1;
                                      ele.update_status = "FILLED";
                                    }
                                  });

                                  let updatedProductLine = [
                                    ...nonDeletedLines,
                                    ...deletedLines,
                                  ];
                                  ...
                                  await product_lines
                                    .findOneAndUpdate(
                                      ...
                                      { product_line: updatedProductLine },
                                      { new: true }
                                    )
                                    .catch((ex) => {
                                      loggerhandler.logger.error(
                                  ...
                                  let allNonDeletedFilled =
                                    nonDeletedLines.length > 0 &&
                                    nonDeletedLines.every(
                                      (e) => e.update_status == "FILLED"
                                    );

                                  if (allNonDeletedFilled) {
                                    await purchase_orders
                                      .findOneAndUpdate(
                                        {
                                          _id: mongoose.Types.ObjectId(
      ...
      if (selected_aware_tokens) {
        for (const item of selected_aware_tokens.selected_tokens) {
          if (!map2.has(mongoose.Types.ObjectId(item.update_aware_token_id))) {
            map2.set(mongoose.Types.ObjectId(item.update_aware_token_id), true);
            ...
            if (materialCertificates && materialCertificateTypes) {
              const available_material_certificates = materialCertificates.map(
                (cert) => {
                  ...
                  return {
                    ...cert._doc,
                    typeName: typeObj ? typeObj.type : null,
                  };
                }
              );
